...
const Filter = ({ filter, setFilter }) => {
  ...
  const [filteredOptions, setFilteredOptions] = useState({
    selectedOption: "type",
    minPrice: filter ? filter.minPrice : 0,
    maxPrice: filter ? filter.maxPrice : 900,
    types: filter ? filter.types : [],
  });
  return (
    <View className="w-full h-full flex flex-col mt-6">
      <View className="w-full pt-2 bg-gray-200 flex flex-row justify-between">
        ...
        {filterOptions.selectedOption === "type" ? (
          <>
            ...
            <PaperButton
              ...
              uppercase={false}
              onPress={() => setFilter(filteredOptions)}
            >
              Apply
            </PaperButton>
          </>
